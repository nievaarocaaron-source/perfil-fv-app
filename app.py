...
if cmj_file and fv_file:
    ...
    st.markdown("<div class='section-title'>📈 Rankings del Equipo</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='section-title'>🗺️ Mapa Fuerza vs Velocidad</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='section-title'>👤 Perfil Individual</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='section-title'>📥 Exportar PDF individual</div>", unsafe_allow_html=True)

    if st.button("Descargar PDF del jugador"):
        pdf = FPDF()
        pdf.add_page()
        pdf.set_font("Arial", size=14)
        pdf.cell(200, 10, txt=f"Perfil F-V – {jugador_sel}", ln=1)

        row = res_df[res_df["Jugador"]==jugador_sel].iloc[0]
        pdf.set_font("Arial", size=11)
        pdf.cell(200, 10, txt=f"F0: {row['F0']:.2f} N", ln=1)
        pdf.cell(200, 10, txt=f"V0: {row['V0']:.2f} m/s", ln=1)
        pdf.cell(200, 10, txt=f"Pmax: {row['Pmax']:.2f} W", ln=1)
